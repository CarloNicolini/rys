"""Cycle 6: validate the rho/phi RYS theory on a pre-norm message-passing solver.

Pipeline for one trained depth:

1. Train a pre-norm (additive residual stream) clause-variable SAT solver.
2. Capture the raw variable residual stream and measure the rho/phi table; test
   the plateau prediction ``1 - CKA ~ 1/2 rho^2 sin^2 phi`` against measured CKA.
3. Sweep strict half-open RYS windows ``(start, end)`` (>= 2 duplicated rounds)
   and record Delta valid-assignment rate on val/test/OOD.
...
All RYS here uses the half-open convention of ``rys.surgery.apply_rys`` and
``rys.residual_force.predict_cka_under_rys`` (duplicated rounds are
``start..end-1``), so the theory and the behavioural sweep share one convention.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("results/sat_theory_validation"))
...
def metrics(logits, labels, mask, cvi, csi, cm) -> dict[str, int]:
...
def train(model, loaders, optimizer, device, epochs) -> list[dict]:
    history = []
    for epoch in range(1, epochs + 1):
        model.train()
        for batch in loaders["train"]:
            cvi, csi, cm = kwargs_on(batch, device).values()
            optimizer.zero_grad(set_to_none=True)
            out = model(clause_variable_ids=cvi, clause_sign_ids=csi, clause_mask=cm, return_round_logits=True)
            terms = [soft_sat_loss(rl, cvi, csi, cm) for rl in out["round_logits"]]
            loss = torch.stack(terms).mean()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
        val = evaluate(model, loaders["val"], device)
        history.append({"epoch": epoch, "val_valid": val["valid_assignment_rate"], "val_bit": val["bit_accuracy"]})
...
def evaluate(model, loader, device, *, window: tuple[int, int] | None = None) -> dict[str, float]:
    model.eval()
    counts = {"valid_correct": 0, "bit_correct": 0, "bit_total": 0, "example_total": 0}
    for batch in loader:
        cvi, csi, cm = kwargs_on(batch, device).values()
        ...
        if window is None:
            ...
            with apply_rys(model, window=window, n_repeats=2):
                logits = model(clause_variable_ids=cvi, clause_sign_ids=csi, clause_mask=cm)["logits"]
        for k, v in metrics(logits, labels, mask, cvi, csi, cm).items():
            counts[k] += v
    return {
...
def capture_states(model, loader, device, *, window: tuple[int, int] | None = None, max_batches: int = 4) -> pd.DataFrame:
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        if window is None:
            trace = model(clause_variable_ids=cvi, clause_sign_ids=csi, clause_mask=cm, return_hidden_states=True)["hidden_states"]
        else:
            with apply_rys(model, window=window, n_repeats=2):
                trace = model(clause_variable_ids=cvi, clause_sign_ids=csi, clause_mask=cm, return_hidden_states=True)["hidden_states"]
...
def strict_windows(n_rounds: int, min_span: int = 2) -> list[tuple[int, int]]:
    """Half-open windows (start, end), end-start >= min_span (>=2 duplicated rounds).

    ``apply_rys`` injects the doubled stream at the input of layer ``end``, so we
    require ``end < n_rounds`` (a downstream round must exist to receive it).
    """
...
def delta_validity_matrix(model, loader, device, n_rounds) -> tuple[pd.DataFrame, float, pd.DataFrame]:
    base = evaluate(model, loader, device)["valid_assignment_rate"]
    mat = np.full((n_rounds, n_rounds), np.nan)
    rows = []
    for start, end in strict_windows(n_rounds):
        rys = evaluate(model, loader, device, window=(start, end))["valid_assignment_rate"]
...
def save_heatmap(matrix: pd.DataFrame, path: Path, *, title: str) -> None:
    ...
    fig.colorbar(im, ax=ax, label="Δ valid assignment rate")
    fig.savefig(path, dpi=180, bbox_inches="tight")
    plt.close(fig)


def make_block_map(model, batch, device, window):
    """Return a closure mapping packed hidden -> post-block packed hidden."""
    cvi, csi, cm = kwargs_on(batch, device).values()
    ...
    var0 = backbone.var_init.view(1, 1, d).expand(b, backbone.config.max_vars, d)
    cl0 = backbone.clause_init.view(1, 1, d).expand(b, backbone.config.max_clauses, d)
    hidden = torch.cat([var0, cl0], dim=1).contiguous()
...
def main() -> None:
    ...
    config = MessagePassingConfig(
        max_vars=max_vars, max_clauses=max_clauses, d_model=args.d_model,
        n_rounds=args.depth, d_mlp=args.d_mlp, pre_norm=True, weight_tied=args.weight_tied,
    )
    model = MessagePassingSatModel(config).to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    history = train(model, loaders, optimizer, device, args.epochs)
